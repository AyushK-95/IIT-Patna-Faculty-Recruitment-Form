...
app.post("/formpages/7", isAuthenticated, (req, res) => {
  ...
  let research_statement = req.body.research_statement;
  let teaching_statement= req.body.teaching_statement ;
  let rel_in= req.body.rel_in ;
  let prof_serv= req.body.prof_serv ;
  let jour_details= req.body.jour_details ;
  let conf_details= req.body.conf_details;
  //research_statement = research_statement ? research_statement.substring(3, research_statement.length - 6) : 'a';
  // teaching_statement = teaching_statement ? teaching_statement.substring(3, teaching_statement.length - 6) : '';
  // rel_in = rel_in ? rel_in.substring(3, rel_in.length - 6) : 'a';
